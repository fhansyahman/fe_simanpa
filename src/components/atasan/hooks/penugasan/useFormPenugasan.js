...
export const useFormPenugasan = (onSuccess) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      if (!formData.nama_penugasan || !formData.jam_masuk || !formData.jam_pulang || !formData.batas_akhir_pulang) {
        Swal.fire({ icon: "warning", title: "Data Tidak Lengkap", text: "Nama penugasan, jam masuk, jam pulang, dan batas akhir pulang wajib diisi", confirmButtonColor: "#F59E0B" });
        return;
      }
