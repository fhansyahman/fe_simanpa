          ...
          <form onSubmit={onSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <input
                  type="time"
                  value={formData.batas_akhir_pulang ? formData.batas_akhir_pulang.substring(0, 5) : formData.jam_pulang.substring(0, 5)}
                  onChange={(e) => setFormData({...formData, batas_akhir_pulang: e.target.value + ':00'})}
                  className="w-full border border-gray-300 p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500"
                  required
                />
                <p className="text-xs text-red-500 mt-1">Batas akhir pulang TANPA TOLERANSI. Jika lewat dari jam ini, presensi pulang DITOLAK otomatis.</p>
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Toleransi Keterlambatan Masuk</label>
                <input
                  type="time"
                  value={formData.toleransi_keterlambatan?.substring(0, 5) || '00:15'}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Batas Terlambat Masuk</label>
                <input
                  type="time"
